...
public class InternerLvLcreator {
	...
	public InternerLvLcreator() {
		 ...
		 * Gegner:
		 * 
		 * ? = Enemy e = Enemy nach links + = Enemy nach rechts - = Enemy nach oben / =
		 * Enemy nach unten
		 * 
		 * 
		 * 
		 * Bouncer:
		 * 
		 * r = Bouncer RECHTS l = Bouncer LINKS u = Bouncer HOCH d = Bouncer RUNTER
		 * 
		 * Doors
		 * 
		 * A = Door blau B = Door gelb C = Door gruen D = Door orange E = Door pink F =
		 * Door silber
		 * 
		 * Z = Key blau Y = Key gelb X = Key gruen W = Key orange V = Key pink U = Key
		 * silber
		 * 
		 */
			...
			for (int i = 0; i < maxSpalten; i++) {
				for (int k = 0; k < maxZeilen; k++) {
					...
					if (s.equals("o")) {
						...
						Controller.addBlock(new Block(i * 50 - oldX, k * 50 - oldY));

					}

					else if (s.equals("A")) {

						Controller.returnDoorBlau().setPos(i * 50 - oldX, k * 50 - oldY);

					} else if (s.equals("B")) {

						Controller.returnDoorGelb().setPos(i * 50 - oldX, k * 50 - oldY);

					} else if (s.equals("C")) {

						Controller.returnDoorGruen().setPos(i * 50 - oldX, k * 50 - oldY);

					} else if (s.equals("D")) {

						Controller.returnDoorOrange().setPos(i * 50 - oldX, k * 50 - oldY);

					} else if (s.equals("E")) {

						Controller.returnDoorPink().setPos(i * 50 - oldX, k * 50 - oldY);

					} else if (s.equals("F")) {

						Controller.returnDoorSilber().setPos(i * 50 - oldX, k * 50 - oldY);

					} else if (s.equals("Z")) {

						Controller.returnKeyBlau().setPos(i * 50 - oldX, k * 50 - oldY);
					} else if (s.equals("Y")) {

						Controller.returnKeyGelb().setPos(i * 50 - oldX, k * 50 - oldY);

					} else if (s.equals("X")) {

						Controller.returnKeyGruen().setPos(i * 50 - oldX, k * 50 - oldY);

					} else if (s.equals("W")) {

						Controller.returnKeyOrange().setPos(i * 50 - oldX, k * 50 - oldY);

					} else if (s.equals("V")) {

						Controller.returnKeyPink().setPos(i * 50 - oldX, k * 50 - oldY);

					} else if (s.equals("U")) {

						Controller.returnKeySilber().setPos(i * 50 - oldX, k * 50 - oldY);

					}

					else if (s.equals("r")) {

						Controller.addBouncerToRight(
								new BouncerToRight(i * 50 - oldX + 40, k * 50 - oldY + 10, globaleSichtbarkeit));
						...
						Controller.addBouncerToLeft(
								new BouncerToLeft(i * 50 - oldX, k * 50 - oldY + 10, globaleSichtbarkeit));

					} else if (s.equals("u")) {

						Controller.addBouncerToUp(
								new BouncerToUp(i * 50 - oldX + 10, k * 50 - oldY, globaleSichtbarkeit));

					} else if (s.equals("d")) {

						Controller.addBouncerToDown(
								new BouncerToDown(i * 50 - oldX + 10, k * 50 - oldY + 40, globaleSichtbarkeit));
